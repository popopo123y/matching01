...
import { Heart, MessageSquare, Shield, AlertCircle, Settings, Search as SearchIcon, Lock, Flame, ArrowLeft, Send, Footprints, Briefcase, GraduationCap, HeartHandshake, User as UserIcon, Smile, MoreHorizontal, Flag, X, Check, Plus, ShieldAlert, Zap, Filter, ArrowUpDown, ChevronDown, UserRound, Edit3, Camera, ChevronRight, CreditCard, HelpCircle, FileText, Users, Eye, Sparkles, Hash, Play, MapPin, Ruler, Weight, Calendar, School, Users2, Wine, Cigarette, Dices, Quote } from 'lucide-react';
...
const ME_USER: UserProfile = {
  ...
  gender: Gender.MALE,
  residence: '東京都',
  bio: 'Genesisへようこそ！これはあなた自身のプロフィールです。真剣にパートナーを探しています。',
  photos: ['https://picsum.photos/seed/me/400/600'],
  tags: ['映画', 'カフェ巡り', 'カメラ', 'サウナ'],
  isPremium: false,
  isVerified: true,
  lastLogin: new Date(),
  birthplace: '神奈川県',
  ...
  marriageHistory: 'なし',
  hasChildren: 'なし',
  marriageIntent: '2~3年以内にしたい',
  wantChildren: '欲しい',
  dateCost: '男性が多めに払う',
  personality: '穏やか、誠実、凝り性',
  cohabitants: '一人暮らし',
  alcohol: 'ときどき飲む',
  gambling: '全くしない',
  smoking: '吸わない',
  hobbies: 'プログラミング、サウナ、映画鑑賞、キャンプ',
  fetishPhilia: '声嗜好',
  fetishYears: '3年以上',
  fetishDesiredPlay: '落ち着いた空間でのトーク'
};
...
const App: React.FC = () => {
      ...
      <div className="absolute inset-0 z-[100] bg-gray-900 flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="sticky top-0 z-20 flex border-b border-white/10 bg-gray-900/95 backdrop-blur-md shadow-lg shadow-black/20">
            <button onClick={() => setFetishSubTab('users')} className={`flex-1 py-4 text-xs font-black uppercase tracking-widest transition-all ${fetishSubTab === 'users' ? 'text-purple-400 border-b-4 border-purple-400 bg-purple-500/10' : 'text-gray-500'}`}>候補者</button>
            <button onClick={() => setFetishSubTab('talk')} className={`flex-1 py-4 text-xs font-black uppercase tracking-widest transition-all ${fetishSubTab === 'talk' ? 'text-purple-400 border-b-4 border-purple-400 bg-purple-500/10' : 'text-gray-500'}`}>匿名トーク</button>
          </div>
          ...
          {fetishSubTab === 'users' && (
            <div className="sticky top-[52px] z-10 bg-gray-900/95 p-3 border-b border-white/5 backdrop-blur-sm">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide no-scrollbar">
                <button onClick={() => setSearchFetishTag(null)} className={`flex-shrink-0 px-4 py-1.5 rounded-full text-[10px] font-black transition-all ${!searchFetishTag ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-500 border border-white/5'}`}>すべて</button>
                {FETISH_TAGS.map(tag => (
                  <button key={tag} onClick={() => setSearchFetishTag(tag)} className={`flex-shrink-0 px-4 py-1.5 rounded-full text-[10px] font-black transition-all whitespace-nowrap ${searchFetishTag === tag ? 'bg-purple-600 text-white shadow-lg shadow-purple-900/50' : 'bg-white/5 text-gray-500 border border-white/5 hover:bg-white/10'}`}>#{tag}</button>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="p-6">
            {fetishSubTab === 'users' ? (
              ...
              <div className="flex flex-col gap-6 pb-24">
                {talkMessages.map(m => (
                  <div key={m.id} className={`flex flex-col ${m.user === 'あなた' ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-[85%] px-5 py-3.5 rounded-3xl text-sm ${m.user === 'あなた' ? 'bg-purple-600 text-white rounded-tr-none shadow-xl shadow-purple-900/20' : 'bg-gray-800 text-gray-200 rounded-tl-none border border-white/5 shadow-lg'}`}>{m.text}</div>
                  </div>
                ))}
                <div ref={chatEndRef} />
              </div>
  ...
  const ProfileItem = ({ icon: Icon, label, value }: { icon?: any, label: string, value?: string | number }) => (
  ...
  const renderProfileDetail = (user: UserProfile | any, source: string) => {
    ...
    // 性癖ルーム以外では性癖関連のタグを非表示にする
    const filteredTags = source === 'fetish' 
      ? user.tags 
      : user.tags.filter((tag: string) => !FETISH_TAGS.includes(tag));

    return (
      <div className="fixed inset-0 z-[1000] bg-white flex flex-col overflow-y-auto pb-32 max-w-md mx-auto animate-in slide-in-from-right duration-300">
        <header className="sticky top-0 left-0 right-0 z-50 flex justify-between items-center p-4 bg-white/80 backdrop-blur-md border-b border-gray-100">
          <button onClick={() => setViewingUser(null)} className="p-2 bg-black/5 rounded-full active:scale-90 transition-transform"><ArrowLeft size={20} /></button>
        ...
        <div className="p-6 space-y-10">
          <section>
            <h3 className="flex items-center gap-2 font-black text-xl mb-4"><Quote size={20} className="text-pink-500" /> 自己紹介</h3>
            <p className="text-gray-600 leading-relaxed text-sm whitespace-pre-wrap bg-gray-50 p-6 rounded-[2rem] border border-gray-100">{user.bio || '--'}</p>
          </section>

          <section>
            <h3 className="flex items-center gap-2 font-black text-xl mb-4"><Hash size={20} className="text-pink-500" /> タグ</h3>
          ...
          <section className="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm">
            <h3 className="font-black text-lg mb-4 flex items-center gap-2"><UserRound size={20} className="text-pink-500" /> 基本情報</h3>
            <div className="divide-y divide-gray-50">
              <ProfileItem icon={MapPin} label="居住地" value={user.residence} />
              <ProfileItem icon={MapPin} label="出身地" value={user.birthplace} />
              <ProfileItem icon={Smile} label="体型" value={user.bodyType} />
              <ProfileItem icon={Ruler} label="身長" value={user.height ? `${user.height}cm` : '--'} />
              <ProfileItem icon={Weight} label="体重" value={user.weight ? `${user.weight}kg` : '--'} />
            </div>
          </section>

          {/* 職業・学歴 */}
          <section className="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm">
            <h3 className="font-black text-lg mb-4 flex items-center gap-2"><Briefcase size={20} className="text-pink-500" /> 職業・学歴</h3>
            <div className="divide-y divide-gray-50">
              <ProfileItem icon={Briefcase} label="職業" value={user.job} />
              <ProfileItem icon={Calendar} label="休日" value={user.holiday} />
              <ProfileItem icon={GraduationCap} label="学歴" value={user.education} />
            </div>
          </section>

          {/* 恋愛・結婚観 */}
          <section className="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm">
            <h3 className="font-black text-lg mb-4 flex items-center gap-2"><HeartHandshake size={20} className="text-pink-500" /> 恋愛・結婚観</h3>
            <div className="divide-y divide-gray-50">
              <ProfileItem label="結婚歴" value={user.marriageHistory} />
              <ProfileItem label="子供の有無" value={user.hasChildren} />
              <ProfileItem label="結婚に対する意思" value={user.marriageIntent} />
              <ProfileItem label="子供が欲しいか" value={user.wantChildren} />
              <ProfileItem label="初回デート費用" value={user.dateCost} />
            </div>
          </section>

          {/* 性格・趣味・ライフスタイル */}
          <section className="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm">
            <h3 className="font-black text-lg mb-4 flex items-center gap-2"><Sparkles size={20} className="text-pink-500" /> 性格・ライフスタイル</h3>
            <div className="divide-y divide-gray-50">
              <ProfileItem label="性格" value={user.personality} />
              <ProfileItem icon={Users2} label="同居人" value={user.cohabitants} />
              <ProfileItem icon={Wine} label="お酒" value={user.alcohol} />
              <ProfileItem icon={Dices} label="ギャンブル" value={user.gambling} />
              <ProfileItem icon={Cigarette} label="タバコ" value={user.smoking} />
              <ProfileItem label="趣味" value={user.hobbies} />
            </div>
          </section>

          {/* 性癖プロフィール（性癖ルームからの遷移時のみ表示） */}
          {source === 'fetish' && user.fetishPhilia && (
        ...
        {!isMe && (
          <div className="sticky bottom-10 left-1/2 -translate-x-1/2 w-full max-w-xs flex gap-4 px-6 z-50">
            <button onClick={() => setViewingUser(null)} className="flex-1 h-14 bg-white border-2 border-gray-100 rounded-full flex items-center justify-center text-gray-400 shadow-xl active:scale-95 transition-all"><X size={28} /></button>
            <button onClick={() => { handleLike(user, source === 'fetish' ? 'fetish' : 'normal'); setViewingUser(null); }} className="flex-1 h-14 bg-pink-500 rounded-full flex items-center justify-center text-white shadow-xl shadow-pink-200 active:scale-95 transition-all"><Heart size={28} fill="white" /></button>
          </div>
        )}
      </div>
    );
  ...
  const renderLanding = () => {
    return (
      <div 
        onClick={() => { setShowLanding(false); setActiveTab('discover'); }}
        className="fixed inset-0 z-[300] bg-gray-900 flex flex-col items-center justify-center p-8 cursor-pointer overflow-hidden"
      >
        ...
        <div className="absolute top-1/4 -left-20 w-80 h-80 bg-purple-600/20 rounded-full blur-[100px] animate-pulse"></div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          <div className="w-24 h-24 bg-white/5 border border-white/10 rounded-full flex items-center justify-center mb-8 shadow-2xl backdrop-blur-xl">
            <Flame className="text-purple-500" size={48} />
          </div>
          <h1 className="text-6xl font-black italic tracking-tighter text-white mb-4 uppercase drop-shadow-2xl">Genesis</h1>
          <div className="h-1 w-20 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full mb-12"></div>
          <p className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-white/40 mb-2 tracking-tighter">性癖特化マッチング</p>
          <p className="text-xs text-purple-400 font-bold uppercase tracking-[0.4em] mb-16 opacity-70">Exclusive Connection Space</p>
          <div className="flex items-center gap-3 px-8 py-5 bg-white text-gray-900 rounded-full font-black text-lg shadow-2xl hover:scale-105 active:scale-95 transition-all">
            <Play size={20} fill="currentColor" />はじめる
          </div>
          <p className="mt-8 text-[10px] text-gray-500 font-bold uppercase tracking-widest opacity-50">Tap to enter the sanctuary</p>
        </div>
      </div>
    );
  };
      ...
      case 'discover':
        return (
          <div className="p-4 flex flex-col gap-4">
            ...
            <CardStack users={baseFemaleUsers} onSwipeRight={(u) => handleLike(u, 'normal')} onSwipeLeft={() => {}} onViewDetail={(u) => { setViewingUser(u); setViewingSource('normal'); }} />
          </div>
        );
      case 'list':
        return (
          <div className="min-h-screen bg-white">
            ...
            <div className="p-4 grid grid-cols-2 gap-4">
              {displayUsers.map(user => (
                <div key={user.id} onClick={() => { setViewingUser(user); setViewingSource('normal'); }} className="group cursor-pointer">
                  <div className="relative aspect-[3/4] rounded-2xl overflow-hidden mb-2 shadow-sm border border-gray-100">
      ...
      case 'community':
        return (
          <div className="p-6 pb-24 h-full overflow-y-auto">
            <h1 className="text-3xl font-black mb-8 tracking-tighter italic text-gray-900">コミュニティ</h1>
            <div className="grid grid-cols-2 gap-4">
              {COMMUNITIES.map(comm => (
                <div key={comm.id} onClick={() => setSelectedCommunityId(comm.id)} className="bg-white rounded-[2.5rem] border-2 border-gray-50 overflow-hidden shadow-sm active:scale-95 transition-all cursor-pointer">
        ...
        return (
          <div className="bg-white min-h-screen pb-24 h-full overflow-y-auto">
            <header className="p-6 flex justify-between items-center bg-white sticky top-0 z-10">
              <h1 className="text-2xl font-black italic text-pink-500 uppercase">マイページ</h1>
              <button className="p-2 bg-gray-50 rounded-full hover:bg-gray-100 transition-colors"><Settings size={22} /></button>
            </header>
            
            <div className="p-6 space-y-10">
              {/* プロフィールメインカード */}
              <div className="bg-white rounded-[3.5rem] p-10 shadow-2xl shadow-pink-100 border border-pink-50 flex flex-col items-center cursor-pointer transition-transform active:scale-95" onClick={() => { setViewingUser(ME_USER); setViewingSource('normal'); }}>
                <div className="relative group">
                  <div className="w-40 h-40 rounded-full border-4 border-white shadow-xl overflow-hidden mb-4 group-hover:scale-105 transition-transform"><img src={ME_USER.photos[0]} className="w-full h-full object-cover" /></div>
                  <div className="absolute bottom-6 right-2 bg-pink-500 text-white p-2 rounded-full border-2 border-white shadow-lg"><Camera size={18} /></div>
                </div>
                <h2 className="text-3xl font-black">{ME_USER.nickname}, {ME_USER.age}歳</h2>
                <div className="flex items-center gap-2 mt-1 text-gray-400 font-bold text-xs uppercase tracking-widest"><MapPin size={12} /> {ME_USER.residence}</div>
                <button className="mt-8 w-full py-4 bg-gray-50 text-gray-400 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-gray-100 transition-colors">プロフィール編集 <ChevronRight size={14}/></button>
              </div>

              {/* 統計セクション */}
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm text-center flex flex-col items-center">
                   <div className="w-10 h-10 bg-blue-50 rounded-full flex items-center justify-center mb-2 text-blue-500"><Footprints size={20} /></div>
                   <p className="text-3xl font-black text-blue-500">1,280</p>
                   <p className="text-[10px] text-gray-400 font-black uppercase mt-1 tracking-widest">足跡</p>
                </div>
                <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm text-center flex flex-col items-center">
                   <div className="w-10 h-10 bg-pink-50 rounded-full flex items-center justify-center mb-2 text-pink-500"><Heart size={20} fill="currentColor" /></div>
                   <p className="text-3xl font-black text-pink-500">256</p>
                   <p className="text-[10px] text-gray-400 font-black uppercase mt-1 tracking-widest">もらったいいね</p>
                </div>
              </div>

              {/* 会員ステータス */}
              <div onClick={() => setIsPremium(!isPremium)} className={`p-8 rounded-[2.5rem] border shadow-sm text-center cursor-pointer transition-all ${isPremium ? 'bg-purple-50 border-purple-200' : 'bg-gray-50 border-gray-100'}`}>
                 <p className={`text-sm font-black uppercase tracking-[0.3em] mb-1 ${isPremium ? 'text-purple-600' : 'text-gray-400'}`}>{isPremium ? 'プレミアム会員' : '無料会員'}</p>
                 <p className="text-[10px] text-gray-400 font-bold leading-relaxed">{isPremium ? '全ての機能を利用可能です' : 'アップグレードして性癖ルームへ'}</p>
              </div>

              {/* 基本情報詳細（マイページ用プレビュー） */}
              <div className="bg-white rounded-[2.5rem] border border-gray-100 p-8 shadow-sm space-y-6">
                 <h3 className="font-black text-xl flex items-center gap-2 border-b border-gray-50 pb-4"><UserRound size={22} className="text-pink-500" /> 詳細プロフィール</h3>
                 
                 <div className="space-y-4">
                   <div className="border-b border-gray-50 pb-4">
                     <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-3 flex items-center gap-2"><Smile size={14} /> 基本情報</p>
                     <div className="space-y-3">
                       <ProfileItem label="居住地" value={ME_USER.residence} />
                       <ProfileItem label="出身地" value={ME_USER.birthplace} />
                       <ProfileItem label="体型" value={ME_USER.bodyType} />
                       <ProfileItem label="身長" value={`${ME_USER.height}cm`} />
                       <ProfileItem label="体重" value={`${ME_USER.weight}kg`} />
                     </div>
                   </div>

                   <div className="border-b border-gray-50 pb-4">
                     <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-3 flex items-center gap-2"><Briefcase size={14} /> 職業・学歴</p>
                     <div className="space-y-3">
                       <ProfileItem label="職業" value={ME_USER.job} />
                       <ProfileItem label="休日" value={ME_USER.holiday} />
                       <ProfileItem label="学歴" value={ME_USER.education} />
                     </div>
                   </div>

                   <div className="border-b border-gray-50 pb-4">
                     <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-3 flex items-center gap-2"><HeartHandshake size={14} /> 恋愛・結婚観</p>
                     <div className="space-y-3">
                       <ProfileItem label="結婚歴" value={ME_USER.marriageHistory} />
                       <ProfileItem label="子供有無" value={ME_USER.hasChildren} />
                       <ProfileItem label="結婚感" value={ME_USER.marriageIntent} />
                       <ProfileItem label="子供欲しいか" value={ME_USER.wantChildren} />
                       <ProfileItem label="デート費用" value={ME_USER.dateCost} />
                     </div>
                   </div>

                   <div>
                     <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-3 flex items-center gap-2"><Sparkles size={14} /> 性格・趣味</p>
                     <div className="space-y-3">
                       <ProfileItem label="性格" value={ME_USER.personality} />
                       <ProfileItem label="同居人" value={ME_USER.cohabitants} />
                       <ProfileItem label="お酒" value={ME_USER.alcohol} />
                       <ProfileItem label="ギャンブル" value={ME_USER.gambling} />
                       <ProfileItem label="タバコ" value={ME_USER.smoking} />
                       <ProfileItem label="趣味" value={ME_USER.hobbies} />
                     </div>
                   </div>
                 </div>
              </div>
              ...
              {/* フッターリンク */}
              <div className="space-y-3">
                <button className="w-full flex justify-between items-center p-6 bg-white border border-gray-100 rounded-3xl text-sm font-bold text-gray-700 hover:bg-gray-50"><span>ヘルプ・ガイド</span><ChevronRight size={18} className="text-gray-300" /></button>
                <button className="w-full flex justify-between items-center p-6 bg-white border border-gray-100 rounded-3xl text-sm font-bold text-gray-700 hover:bg-gray-50"><span>利用規約・プライバシーポリシー</span><ChevronRight size={18} className="text-gray-300" /></button>
              </div>
            </div>
          </div>
        );
  ...
  return (
    <div className="max-w-md mx-auto h-screen bg-gray-50 flex flex-col shadow-2xl overflow-hidden relative font-sans">
      ...
      {!showLanding && !viewingUser && !selectedCommunityId && !selectedRole && (
        <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/95 backdrop-blur-md border-t border-gray-100 flex justify-around items-center h-20 px-1 z-40">
          {NAVIGATION_ITEMS.map(item => (
            <button key={item.id} onClick={() => { setActiveTab(item.id); setSelectedRole(null); setSelectedCommunityId(null); }} className={`flex flex-col items-center justify-center gap-1 flex-1 transition-all ${activeTab === item.id ? (activeTab === 'fetish' ? 'text-purple-600' : 'text-pink-500') : 'text-gray-300'}`}>
